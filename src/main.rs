...
let mut available_roles = vec![Top, Jungle, Mid, Bottom, Support];
    ...
    for player_name in players{
        ...
        let champ_pool: ChampPool = from_str(&read_to_string(&t).map_err(|_| format!("File {} not found", t.to_string_lossy()))?).map_err(|_|format!("Failed to parse the file {}",t.to_string_lossy()))?;
        let role = choose_random(&mut available_roles).expect("Seems like my bounds check failed :   ^)");
        ...
        match choose_random(&mut champion_list){
            Some(champion) =>pl.push(Player{ name: player_name.to_owned(), position: role, champion }),
            None => {
                println!("The player {} is a Wukong irl, and cannot play a champion from their role", player_name);
                return Ok(())
            },
        }
    };
    pl.iter().for_each(|p| println!("{} IN {:?} WITH {}", p.name, p.position, p.champion));
    Ok(())
}
